...
// Cap nhat profile cua user (khong can admin)
router.put("/profile/update", authMiddleware, async (req, res) => {
  try {
    ...
    if (updateData.full_name !== undefined) {
      ...
      user.full_name = updateData.full_name.trim();
    ...
    if (
      updateData.current_password !== undefined ||
      updateData.new_password !== undefined
    ) {
      // Kiểm tra có đủ cả mật khẩu cũ và mới không
      ...
      const isMatch = await user.isPasswordMatch(updateData.current_password);
      if (!isMatch) {
        return res.status(400).json({
      ...
      user.password = updateData.new_password;
    ...
    // Tra ve thong tin da cap nhat (khong bao gom password)
    res.json({
